...
class BaseTestCase(TestCase):
    ...
    def setUp(self):
        ...
        db.session.add(User("admin", "adtest@gmail.com", "admin"))
        db.session.commit()
...
class FlaskTestCase(BaseTestCase):
    ...
    # Ensure that posts show 9999up on the main page
...
class UsersViewsTests(BaseTestCase):
    ...
    def test_user_registration(self):
        with self.client:
            response = self.client.post(
                '/register/',
                data=dict(username="alok", email="alok@gmail.com", password="123123", confirm="123123"),
                follow_redirects=True
            )
            self.assertIn(b'Welcome to Flask!', response.data)
            self.assertTrue(current_user.name == "alok")
            self.assertTrue(current_user.is_active())
